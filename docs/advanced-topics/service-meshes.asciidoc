    ...
    podTemplate:
      metadata:
        annotations:
          ...
          config.alpha.linkerd.io/proxy-wait-before-exit-seconds: "65" <3>
      spec:
...
<3> Until Native sidecars is available as an alpha feature in Kubernetes 1.28, this annotation is necessary to prevent closing of active, open connections during the shutdown of the pod.
<4> Enable service account token mounting to provide service identity (only required to enable mTLS if service account auto-mounting is disabled).
